...
public class UserService {
    ...
    public User getCurrentUser() { //the jwt contains diff set of claims and in that sub field is one of the claims, the sub field contains the id of the user
        String sub = ((Jwt) (SecurityContextHolder.getContext().getAuthentication().getPrincipal())).getClaim("sub");// same as our usercontroller once we get the principal ve to cast it with the jwt.
    ...
    public void subscribeUser(String userId) {
        //retrieve the current user and add the user id to the subscribed user's set
        User currentUser = getCurrentUser();
